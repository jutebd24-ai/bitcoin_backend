...
class SMSService {
  ...
  private initializeClient() {
    ...
    // Validate credentials format before initializing Twilio client
    ...
    if (!accountSid.startsWith('AC')) {
      console.log('SMS Service not configured - invalid Twilio Account SID format (must start with AC)');
      return;
    }

    if (!fromNumber.startsWith('+')) {
      console.log('SMS Service not configured - invalid phone number format (must start with +)');
      return;
    }
    ...
    try {
      ...
      console.log('SMS Service initialization failed:', error instanceof Error ? error.message : 'Unknown error');
    }
  }
