...
// Elite subscription middleware - only Elite tier is supported
export const requireSubscription = (_minimumTier?: string) => {
  return (req: any, res: Response, next: NextFunction) => {
    ...
    // Only Elite subscription tier is supported
    if (req.user.subscriptionTier !== 'elite') {
      return res.status(403).json({
        message: 'Elite subscription required',
    ...
    // Also check that subscription is active
    if (req.user.subscriptionStatus !== 'active') {
      return res.status(403).json({
        message: 'Active Elite subscription required',
